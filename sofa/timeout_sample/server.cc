...
#include "sandbox/sofa/timeout_sample/sleep_service.pb.h"
...
public:
  ...
  virtual void SleepWithServiceTimeout(
    google::protobuf::RpcController* controller,
    const sofa::pbrpc::test::SleepRequest* request,
    sofa::pbrpc::test::SleepResponse* response,
    google::protobuf::Closure* done) {
    SLOG(NOTICE,
      "SleepWithServiceTimeout(): request sleep time: %d",
      request->sleep_time()
    );
    Sleep(controller, request, response, done);
  }

  virtual void SleepWithMethodTimeout(
    google::protobuf::RpcController* controller,
    const sofa::pbrpc::test::SleepRequest* request,
    sofa::pbrpc::test::SleepResponse* response,
    google::protobuf::Closure* done) {
      SLOG(NOTICE,
        "SleepWithMethodTimeout(): request sleep time: %d",
        request->sleep_time()
      );
      Sleep(controller, request, response, done);
  }
